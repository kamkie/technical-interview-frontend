...
| Session, auth, CSRF, or localization behavior         | Backend contract artifacts and executable tests                                         | Route guards, smoke notes, and affected docs agree with the imported backend contract                                                                                                                  |
...
| AI procedure guidance                                 | Focused files under `.agents/references/` and reusable prompts under `.agents/prompts/` | `AGENTS.md` links to focused references and prompt recipes instead of duplicating full rules; `.agents/references/references-rules.md` and `.agents/prompts/README.md` keep their sets non-duplicative |
...
- `AGENTS.md` keeps root AI gates, truth priority, and pointers to focused AI references.
- `.agents/prompts/README.md` indexes reusable prompt recipes; prompts shape context for named tasks but do not own durable policy.
