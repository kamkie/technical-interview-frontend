...
This file owns active-plan execution rules for delegated milestone and spec slices in this frontend repository. Use it when the user asks to implement an active plan.

## Execution Contract

When the current user request asks to implement an active plan, the plan is the execution contract. The coordinator executes the next eligible plan slice, not a different interpretation of the roadmap or a new direct-implementation path.

The coordinator may update coordinator-owned plan or status documents, assign workers, review worker output, run validation, resolve integration issues, and create plan-authorized commits. Milestone and spec implementation must be delegated to workers with explicit file ownership and scoped validation requirements.

## Dependency Order

Execute in dependency order:

1. Select the next `Ready` milestone, spec, or task slice from the active plan.
2. Assign a planning worker when the slice needs a handoff or the plan requires one.
...
Stop before implementation only when the next `Ready` slice is actually blocked by a decision or external condition that cannot be resolved from the current request, backend contract, executable tests, owner documents, or plan.
...
- worker prompts and handoff review
- final validation selection and reporting
- plan-authorized commits when the current request and checkpoint allow them

The coordinator does not implement milestone or spec tasks directly when the active plan calls for delegated execution. Worker prompts must include repository path, relevant instructions, read-only context, write scope, validation, stop conditions, and output requirements.
...
- the next executable slice lacks clear owner files or validation
...
- implementation would require editing outside the active slice's authorized scope
...
At the end of a plan run or slice handoff:
