...
- Session menus and admin toolbars: sign-in and sign-out actions lead their menus, with the session endpoint, cookie, and CSRF metadata behind a compact connection-details disclosure. The admin list Refresh buttons are removed (the `M-WORKFLOW-002` decision): mutations refresh their lists in place, URL-driven query changes re-fetch, and error states carry retry affordances, so a standing Refresh control added clutter without a workflow it owned; a browser reload remains the manual escape hatch for cross-admin staleness.
